...
class BoundsTest extends PHPUnit_Framework_TestCase
{
   ...
   {
      $this->sw = new LatLong(32.91663, -96.982841);
      $this->ne = new LatLong(32.96302, -96.919495);
      $this->bounds = new Bounds($this->sw, $this->ne);
      $this->loc_a = new LatLong(32.918593, -96.958444);  // inside bounds
      $this->loc_b = new LatLong(32.914144, -96.958444);  // outside bounds
      
      $this->cross_meridian = Bounds::normalize(array(30, 170),array(40, -170));
      $this->inside_cm = new LatLong(35, 175);
      $this->inside_cm2 = new LatLong(35, -175);
      $this->east_of_cm = new LatLong(35, -165);
      $this->west_of_cm = new LatLong(35, 165);
   }
   
   public function testConstructorExceptionOnInvalidArguments()
   {
      $this->setExpectedException('InvalidArgumentException');
      $res = new Bounds(array(30, 170), array(40, -170));
   }
   
   public function testEquality()
   {
      $this->assertEquals(new Bounds($this->sw, $this->ne), new Bounds($this->sw, $this->ne));
      
      $bounds = new Bounds($this->sw, $this->ne);
      $this->assertTrue($bounds->equal(new Bounds($this->sw, $this->ne)));
   }
   
   public function testNormalize()
   {
      $res = Bounds::normalize($this->bounds);
      $this->assertEquals($res, $this->bounds);
      
      $res = Bounds::normalize($this->sw, $this->ne);
      $this->assertEquals($res, new Bounds($this->sw, $this->ne));
      
      $res = Bounds::normalize(array($this->sw, $this->ne));
      $this->assertEquals($res, new Bounds($this->sw, $this->ne));
      
      $res = Bounds::normalize(array($this->sw->lat, $this->sw->lng), array($this->ne->lat, $this->ne->lng));
      $this->assertEquals($res, new Bounds($this->sw, $this->ne));
      
      $res = Bounds::normalize(array(array($this->sw->lat, $this->sw->lng), array($this->ne->lat, $this->ne->lng)));
      $this->assertEquals($res, new Bounds($this->sw, $this->ne));
   }
   ...
   public function testCreationFromCircle()
   {
       ...
       $inside = new LatLong(32.9695270000, -96.9901590000);
       $outside = new LatLong(32.8951550000, -96.9584440000);
   ...
   {
      $sw = new LatLong(32, -96);
      $ne = new LatLong(40, -70);
      $bounds = new Bounds($sw, $ne);
      
      $this->assertEQuals(new LatLong(8, 26), $bounds->toSpan());
   }
   
   public function testBoundsToSpanCrossingPrimeMeridian()
   {
      $sw = new LatLong(20, -70);
      $ne = new LatLong(40, 100);
      $bounds = new Bounds($sw, $ne);
      
      $this->assertEQuals(new LatLong(20, 170), $bounds->toSpan());
   }
   
   public function testBoundsToSpanCrossingPrimeDateline()
   {
      $sw = new LatLong(20, 100);
      $ne = new LatLong(40, -70);
      $bounds = new Bounds($sw, $ne);
      
      $this->assertEQuals(new LatLong(20, 190), $bounds->toSpan());
   }
